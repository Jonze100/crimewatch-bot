...
from whale import scan_whale_activity, TOKEN_REGISTRY
...
snoozed = set()
scan_stats = {"last_scan": "Never", "tokens_scanned": 0, "alerts_sent": 0, "last_whale_scan": "Never"}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Two independent scanners running 24/7:\n\n"
        "📊 *Scanner 1 — Market signals (every 10 mins):*\n"
        "  • Negative funding rate\n"
        "  • Heavy short positioning\n"
        "  • OI building on low volume\n"
        "  • Price coiling\n"
        "  • Basis spread anomalies\n\n"
        "🐋 *Scanner 2 — Whale activity (every 30 mins):*\n"
        "  • Large withdrawals from Binance wallets\n"
        "  • Single tx >3% of circulating supply\n"
        "  • New wallet accumulation\n\n"
        "*Alert thresholds:*\n"
        "  💎 Score 65–74 — High conviction\n"
        "  🚀 Score 75+ — Extreme conviction\n"
        "  🚨 Pump signal — All signals aligned\n"
        "  🐋 Whale alert — Large Binance outflow\n\n"
        "• /scan SYMBOL — Manual scan\n"
        "• /status — Bot status\n"
        "• /snooze SYMBOL — Mute alerts\n\n"
        "Sit back. I'll alert you when something's cooking.",
        parse_mode="Markdown"
    )
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"  • Tokens monitored: {len(TOKEN_REGISTRY)}\n\n"
...
def fmt_whale(alert):
    ...
    lines = [
        ...
        f"A large withdrawal just left Binance:",
        f"  • Amount: *{amount:,.0f} {symbol}*",
        f"  • Supply removed: *{pct:.1f}% of circulating supply*",
        f"  • Destination wallet: `{short_whale}`",
        f"  • Tx: [{short_tx}]({explorer}/tx/{tx})", "",
        f"*What this means:*",
        f"  • Supply shock incoming — less {symbol} on exchange",
        f"  • If shorts are heavy, this could trigger a squeeze",
        f"  • Watch for funding rate to flip negative", "",
        f"*Action:* Run /scan {symbol}USDT for full market analysis",
        f"[View on Binance](https://www.binance.com/en/futures/{symbol}USDT)"
    ]
    return "\n".join(lines)
...
async def whale_scan_loop(app):
    while True:
        await asyncio.sleep(30 * 60)  # every 30 mins
        if not ETHERSCAN_API_KEY:
            logger.warning("No ETHERSCAN_API_KEY set, skipping whale scan")
            continue
        try:
            scan_stats["last_whale_scan"] = datetime.datetime.now().strftime("%H:%M:%S")
            logger.info("Running whale scan...")
            findings = await scan_whale_activity(ETHERSCAN_API_KEY)
            for alert in findings:
                ...
                if tx_hash and tx_hash not in last_whale_alerted:
                    ...
                    logger.info(f"Whale alert sent: {alert['symbol']} {alert['pct_supply']:.1f}%")
        except Exception as e:
...
async def main():
    ...
    logger.info("Crime Watch — market + whale scanners running")
    async with app:
        await app.start()
