...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in ALERT_CHAT_IDS: ALERT_CHAT_IDS.append(chat_id)
    await update.message.reply_text(
        "🔮 *Crime Watch — Full Pre-Pump Intelligence*\n\n"
        "Three autonomous scanners. Every Binance futures token.\n\n"
        "📊 *Static scanner (every 10 mins):*\n"
        "  • Funding rate, L/S ratio, OI vs volume\n"
        "  • Price coiling, basis spread\n\n"
        "⚡ *Dynamic scanner (every 10 mins):*\n"
        "  • OI spike — sudden position buildup\n"
        "  • Funding flip — shorts just took over\n"
        "  • Volume explosion — unusual activity\n"
        "  • Shorts piling in — L/S dropping fast\n"
        "  • Perfect coil — flat price + OI + volume spike\n\n"
        "🐋 *Whale scanner (every 60 mins):*\n"
        "  • Large withdrawals from Binance hot wallets\n\n"
        "*Alert levels:*\n"
        "  ⚡ Dynamic alert — something just changed\n"
        "  💎 Score 65+ — High conviction\n"
...
async def scan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: await update.message.reply_text("Usage: /scan SYMBOL\nExample: /scan BTCUSDT"); return
...
async def snooze_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: await update.message.reply_text("Usage: /snooze SYMBOL"); return
...
async def unsnooze_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: await update.message.reply_text("Usage: /unsnooze SYMBOL"); return
    s = context.args[0].upper()
    if not s.endswith("USDT"): s += "USDT"
    snoozed.discard(s)
...
def fmt(r):
    if r.get("error"): return f"❌ *{r['symbol']}*\n{r['error']}"
    sc=r["crime_score"]; dsc=r.get("dynamic_score",0)
    ...
    score_line=f"*{sc}/100 ({lbl})*" + (f"  _(+{dsc} dynamic)_" if dsc>0 else "")
    lines=[f"🔮 *{r['symbol']}* [Binance]  |  {score_line}","",
    ...
    if r.get("risk_signals"):
        ...
        lines.append("")
    lines.append(pump_line); lines.append("")
...
def fmt_whale(alert):
    ...
    return "\n".join([f"🐋 *WHALE ALERT — ${symbol}* [{urgency}]","",
...
async def send_alert(app, result, level):
    labels={"dynamic":"⚡ DYNAMIC ALERT — Something just changed","high":"💎 HIGH CONVICTION","extreme":"🚀 EXTREME SETUP","pump":"🚨 LIVE PUMP SIGNAL"}
    text=f"{labels.get(level,'🔔')} — *{result['symbol']}*\n\n"+fmt(result)
    scan_stats["alerts_sent"]+=1
    for cid in ALERT_CHAT_IDS:
        try: await app.bot.send_message(int(cid),text,parse_mode="Markdown",disable_web_page_preview=True)
        except Exception as e: logger.error(f"Alert failed {cid}: {e}")

async def send_whale_alert(app, alert):
    scan_stats["alerts_sent"]+=1
    for cid in ALERT_CHAT_IDS:
        try: await app.bot.send_message(int(cid),fmt_whale(alert),parse_mode="Markdown",disable_web_page_preview=True)
        except Exception as e: logger.error(f"Whale alert failed {cid}: {e}")
...
async def market_scan_loop(app):
    ...
    while True:
        try:
            ...
            async with aiohttp.ClientSession(headers={"User-Agent":"CrimeWatch/1.0"},timeout=aiohttp.ClientTimeout(total=20)) as session:
            ...
            for symbol in symbols:
                ...
                try:
                    result=await scan_token(symbol)
                    sc=result.get("crime_score",0); pump=result.get("pump_signal",False)
                    dyn=result.get("dynamic_alert",False); prev=last_alerted.get(symbol,0)
                    if pump and sc>=MIN_ALERT_SCORE:
                        await send_alert(app,result,"pump"); last_alerted[symbol]=sc
                    elif sc>=75 and sc>prev+8:
                        await send_alert(app,result,"extreme"); last_alerted[symbol]=sc
                    elif sc>=MIN_ALERT_SCORE and sc>prev+8:
                        await send_alert(app,result,"high"); last_alerted[symbol]=sc
                    elif dyn and sc>=45 and sc>prev+5:
                        await send_alert(app,result,"dynamic"); last_alerted[symbol]=sc
                    await asyncio.sleep(0.3)
                except Exception as e: logger.error(f"Scan error {symbol}: {e}")
            logger.info("Market scan complete.")
        except Exception as e: logger.error(f"Market cycle error: {e}")
        ...
        try:
            ...
            for alert in findings:
                ...
                if tx_hash and tx_hash not in last_whale_alerted:
                    last_whale_alerted.add(tx_hash); await send_whale_alert(app,alert)
            logger.info(f"Whale scan done. {len(findings)} alerts.")
        except Exception as e: logger.error(f"Whale cycle error: {e}")
...
async def main():
    ...
    for cmd,fn in [("start",start),("scan",scan_cmd),("snooze",snooze_cmd),("unsnooze",unsnooze_cmd),("status",status_cmd)]:
        app.add_handler(CommandHandler(cmd,fn))
    logger.info("Crime Watch — static + dynamic + whale scanners active")
    async with app:
        await app.start()
